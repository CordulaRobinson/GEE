...
"""
# Mine 1.
lon_ll = 25.79 ; lon_tr = 25.83 ; lat_ll = -10.79 ; lat_tr= -10.77
...
if(hyper.size().getInfo() != 0):
    print('Found ', hyper.size().getInfo(),' Hyperion measurements, retrieving and re-gridding them now.....')
    # convert the entire dataset to a list for GEE (server side)
    hyper_list = hyper.toList(hyper.size())
    for i in range(hyper.size().getInfo()):
        ...
        date_aquisition = dt.datetime(year=1970,month=1,day=1) + dt.timedelta(seconds=second/1000)
        ...
        fname = os.path.join(os.getcwd(),'Hyperion_'+str(lon_ll)+'_'+str(lat_ll)+'_'+str(lon_tr)+'_'+str(lat_tr)+'.nc')
