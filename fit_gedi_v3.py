# -*- coding: utf-8 -*-
"""
Created on Tue Jan 11 16:18:23 2022
...
l1bdir = 'C:/Users/e.conway/Research/GEDI/L1B/'

#file = os.path.join(l1bdir,'GEDI01_B_2021072074834_O12742_03_T07685_02_005_02_V002.h5')
file = os.path.join(l1bdir,'GEDI01_B_2020287194837_O10409_03_T07670_02_005_02_V002.h5')
...
for beam in beams:
    ...
    #print(np.nanmax(wfLat_full),np.nanmin(wfLat_full))
    ...
    """
    wv_zeroed = wv_form - np.nanmean(wv_form[0:10])
    
    plt.plot(wv_zeroed,stretch,label='original')
    for i in range(len(pos_maxima)):
        #plt.vlines(pos_maxima[i],0,wv_maxima[i],color='red')
        plt.hlines(stretch[int(pos_maxima[i])],0,wv_maxima[i],color='red')
    for i in range(len(pos_minima)):
        #plt.vlines(pos_minima[i],0,wv_minima[i],color='blue')
        plt.hlines(stretch[int(pos_minima[i])],0,wv_minima[i],color='blue')
    plt.legend()
    plt.show()
    """
