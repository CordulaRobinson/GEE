# Imports and Connected to Google Earth Engine (GEE)
import ee
import geemap
import math
import sys
...
# Create a CSV file that contains a row for each segment of the selected region with the coordinates and status
header = ['min lon', 'min lat', 'max lon', 'max lat', 'status']

# Open the file in the write mode
# gee_dcr_squareSize_lonMin_latMin_lonMax_latMax
f = open('/scratch/nason.e/gee/results_csv/'+'gee_dcr_'+sys.argv[1]+'_'+sys.argv[2]+'_'+sys.argv[3]+'_'+sys.argv[4]+'_'+sys.argv[5]+'.csv', 'a', newline='')
...
for region in regions:
    ...
    lon_min = ee.List(coords.get(0)).get(0).getInfo()
    lon_max = ee.List(coords.get(1)).get(0).getInfo()
    lat_min = ee.List(coords.get(0)).get(1).getInfo()
    lat_max = ee.List(coords.get(2)).get(1).getInfo()
...
# Alternate method for downloading Geotiff Files: saves to Google Drive
# Requires extra steps to access the files, and takes longer in general
# num = 1
# for region in regions:
#     #g = region.geometry()
#     # coords = ee.List(region.coordinates().get(0))
...
#                                          folder='ee multi export jobs',
#                                          scale=30,
#                                          region=region,
#                                          #fileNamePrefix=str(lon_min)+'_'+str(lat_min)+'_'+str(lon_max)+'_'+str(lat_max),
#                                          fileNamePrefix='region_'+str(num),
#                                          crs='EPSG:4326',
#                                          fileFormat='GeoTIFF')
#     task.start()
#     num = num+1
